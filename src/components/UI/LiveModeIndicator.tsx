...
export default function LiveModeIndicator({ variant = 'banner', className = '' }: LiveModeIndicatorProps) {
  ...
  switch (variant) {
    ...
    case 'inline':
      return (
        <div className={`bg-red-100 text-red-800 px-3 py-1 rounded-full text-sm font-medium inline-flex items-center ${className}`}>
    ...
    case 'badge':
      return (
        <span className={`ml-2 px-2 py-0.5 bg-red-100 text-red-800 text-xs rounded-full ${className}`}>
          LIVE MODE
        </span>
      );
